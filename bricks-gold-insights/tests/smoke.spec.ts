...
const PLUGIN_PAGES: Record<string, PluginPage> = {
  analytics: {
    navLabel: 'Analytics',
    path: '/analytics',
    expectedTexts: ['Gold Layer Analytics', 'Users by Country (Top 15)'],
  },
  lakebase: {
    navLabel: 'Lakebase',
    path: '/lakebase',
...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'Bricks Gold Insights' }),
  ).toBeVisible();
  await expect(page.getByText('Data Sources')).toBeVisible();

  await expect(page.getByRole('link', { name: 'Overview' })).toBeVisible();
  for (const [, plugin] of enabledPages) {
    const linkName = plugin.navLabel === 'Analytics' ? 'Analytics' : plugin.navLabel;
